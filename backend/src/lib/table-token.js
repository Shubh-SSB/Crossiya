...
function getSecret() {
  return loadOrCreateSecrets().tableAccessSecret ?? "";
}

function buildPayload(tableNumber, issuedAt) {
  return `${TOKEN_VERSION}.${tableNumber}.${issuedAt}`;
}

export function createTableToken(tableNumber, issuedAt = Date.now()) {
...
export function verifyTableToken(tableNumber, token) {
  const secret = getSecret();
